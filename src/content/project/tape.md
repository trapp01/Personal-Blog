...
The plumbing is in: the config, the Alpaca paper adapter, the SQLite journal, the cost model, the model layer, and manual orders end to end, behind 135 tests that never touch the network. There's no morning briefing yet, which is sort of the main feature, so there's a way to go.
